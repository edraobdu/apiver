...
But a link that has to stay valid on its own timeline — an email sent today that a customer might click
in six months, a webhook payload, a generated PDF — can't ride an `Alias` that may have moved on by the
time anything follows it. For those, resolve against a specific `Version`'s namespace directly
