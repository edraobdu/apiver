...
def test_each_docs_page_points_at_its_own_schema_not_a_sibling_versions(client, docs_path, expected_schema_path):
    ...
    a replacement — see config/urls.py). The Base Version otherwise keeps bare,
    unnamespaced route names (ADR 0001 item 4) to match what the pre-existing
    project already used — which, left alone, would collide: two different
    routes named "schema" (or "docs"), and Django's reverse() silently resolving
    to whichever was registered last. `apiver migrate` avoids this automatically
    (ticket 22): a discovered schema route is always named "v1-schema" — a new
    version's schema is never the same route as whatever it was adopted from —
    and every discovered Swagger/Redoc view is named "v1-docs" and points its
    own url_name at the qualified schema name. See api/v1/registry.py.
    """
