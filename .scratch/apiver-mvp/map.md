...
- **0.1 build slices.** The actual implementation tickets. Wait on 08 — and the reference project specifically waits on [11 — Reference project: shape and structure](issues/11-reference-project-shape.md) as well.
- **Gating semantics in detail.** Exact `Deprecation`/`Sunset` header formats and the unknown-version-gating behavior. Alias declaration, mounting, and schema handling are now settled — [07](issues/07-public-api-surface.md). Downstream of the manifest schema (08).
