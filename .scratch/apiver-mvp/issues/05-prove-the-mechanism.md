...
**Capture:** spike code (`spike/`, `tests/`, `pytest.ini`, `README.md`) committed to the throwaway branch `prototype/05-mechanism-spike`, out of `master`. The `.venv/` used to run it is gitignored and not part of that commit; recreate with `uv venv && uv pip install --python .venv/bin/python django djangorestframework drf-spectacular pytest pytest-django`.
